		...
		(
				...
				(function() {
					...
					function isValid(m) {
						if (m._isValid == null) {
							...
							var parsedParts = some$1.call(
									flags.parsedDateParts, function(i) {
										return i != null;
									});
					...
					function calendar(key, mom, now) {
						...
						return isFunction(output) ? output.call(mom, now)
								: output;
					}
					...
					function handleStrictParse(monthName, format, strict) {
						...
						if (strict) {
							if (format === 'MMM') {
								ii = indexOf$1
										.call(this._shortMonthsParse, llc);
								return ii !== -1 ? ii : null;
							} else {
					...
					function monthsShortRegex(isStrict) {
						if (this._monthsParseExact) {
							...
							return this._monthsShortStrictRegex && isStrict ? this._monthsShortStrictRegex
					...
					function localeWeekdaysShort(m) {
						return (m) ? this._weekdaysShort[m.day()]
								: this._weekdaysShort;
					}
					...
					function localeWeekdaysMin(m) {
						return (m) ? this._weekdaysMin[m.day()]
								: this._weekdaysMin;
					}
					...
					function handleStrictParse$1(weekdayName, format, strict) {
						...
						if (strict) {
							if (format === 'dddd') {
								...
								ii = indexOf$1
										.call(this._minWeekdaysParse, llc);
								return ii !== -1 ? ii : null;
							}
						} else {
							if (format === 'dddd') {
					...
					function getSetISODayOfWeek(input) {
						...
						if (input != null) {
							var weekday = parseIsoWeekday(input, this
									.localeData());
							return this.day(this.day() % 7 ? weekday
									: weekday - 7);
						} else {
							return this.day() || 7;
						}
					...
					function weekdaysMinRegex(isStrict) {
						if (this._weekdaysParseExact) {
							...
							return this._weekdaysMinStrictRegex && isStrict ? this._weekdaysMinStrictRegex
					...
					function offsetFromString(matcher, string) {
						...
						return minutes === 0 ? 0 : parts[0] === '+' ? minutes
								: -minutes;
					}
					...
					function isDaylightSavingTimeShifted() {
						...
						if (c._a) {
							var other = c._isUTC ? createUTC(c._a)
									: createLocal(c._a);
					...
					function getCalendarFormat(myMoment, now) {
						...
						return diff < -6 ? 'sameElse' : diff < -1 ? 'lastWeek'
								: diff < 0 ? 'lastDay' : diff < 1 ? 'sameDay'
										: diff < 2 ? 'nextDay'
												: diff < 7 ? 'nextWeek'
														: 'sameElse';
					}
					...
					function isBetween(from, to, units, inclusivity) {
						...
						return (inclusivity[0] === '(' ? this.isAfter(from,
								units) : !this.isBefore(from, units))
					...
					function listWeekdaysImpl(localeSorted, format, index,
						...
						var locale = getLocale(), shift = localeSorted ? locale._week.dow
								: 0;
